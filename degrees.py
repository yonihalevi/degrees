...
def shortest_path(source, target):
    ...
    Design:
    1. create queue frontier. create set explored
    2. add source to the frontier
    3. forever
        3.a dequeue from frontier. if target- halt.
        3.b get neighbors of item.
    ...
    backtrace the solution path to it
    ...
    while not frontier.empty():
        ...
        explored[explored_node.state] = explored_node

        if explored_node.state == target:
            solution_node = explored_node
            break
        else:
            for (movie_id, person_id) in neighbors_for_person(explored_node.state):
                new_node = Node(person_id, explored_node.state, movie_id)
                if person_id == target:
                    solution_node = new_node
                    return solution(solution_node, explored)
...
def solution(node, explored_set):
    solution_path = []

    while node.parent is not None:
        solution_path.append((node.action, node.state))
        node = explored_set[node.parent]
